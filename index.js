...
function getYears(arr,cb) {
    const yearData = arr.map((item) =>{  //map will create a new array bacsed on the callnback being used 
        return item.Year;
    });
    return cb(yearData);
}
...
function getWinners(arr){
    const winners = arr.map(function(item){
...
function getWinnersByYear(getWinnersCB, getYearsCB){
    ...
    getWinnersCB.forEach(function(item,index){
        winnerString.push(`In ${getYearsCB[index]}, ${item} won the world cup!`)
    });
    return winnerString;
}
